...
#### HTTP vs HTTPS
- **Desenvolvimento local**: HTTP (`http://localhost:5000`) - Câmera permitida
- **Rede local**: HTTPS obrigatório (`https://IP:5000`) - Navegadores bloqueiam câmera em HTTP não-localhost
- **Produção**: HTTPS com certificado TLS válido (Let's Encrypt)
- **HSTS**: Adicionar header Strict-Transport-Security

#### Câmera e MediaDevices API
Navegadores exigem contexto seguro para `navigator.mediaDevices.getUserMedia()`:

| Contexto | Câmera | Uso |
|----------|--------|-----|
| `http://localhost` | ✅ Permitido | Desenvolvimento |
| `http://127.0.0.1` | ❌ Bloqueado | Não use |
| `http://192.168.x.x` | ❌ Bloqueado | Rede HTTP |
| `https://*` | ✅ Permitido | Rede/Produção |

**Solução implementada**: Script `run_https.py` gera certificado SSL auto-assinado para desenvolvimento em rede.
...
### RS011 - Permissões de Câmera
- **Armazenamento**: Permissões persistentes no navegador (domínio)
- **Revogação**: Usuário pode revogar via configurações do navegador
- **Fallback**: Sistema funciona sem câmera (análise por upload de imagem futuro)
- **Privacidade**: Frames processados localmente, nada enviado ao servidor
...
- [x] Configurar HTTPS (certificado auto-assinado para rede)
- [ ] Adicionar headers de segurança
- [ ] Testar permissões de câmera em diferentes navegadores
...
3. **Certificado auto-assinado** - Substituir por Let's Encrypt em produção
