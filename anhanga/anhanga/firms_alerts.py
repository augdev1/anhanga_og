...
# Default to VIIRS SNPP if no key (some endpoints work without key for testing)
...
def fetch_firms_alerts(
    ...
    try:
        ...
        # Build API URL
        # Format: /api/area/csv/{source}/{key}/{bbox}/{start_date}/{end_date}
        bbox_str = f"{min_lon},{min_lat},{max_lon},{max_lat}"
        
        # If we have an API key, use it; otherwise try public endpoints
        if FIRMS_API_KEY:
            url = f"{FIRMS_API_URL}/{DEFAULT_SOURCE}/{FIRMS_API_KEY}/{bbox_str}/{start_date}/{end_date}"
        else:
            # Try without key (some sources may work)
            url = f"{FIRMS_API_URL}/{DEFAULT_SOURCE}/{bbox_str}/{start_date}/{end_date}"
