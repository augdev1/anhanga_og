...
|----------|---------------------|
| User | username, name, password_hash, user_type |
| Report | lat, lon, nivel, count, created_at |
| AuthToken | token, username, created_at |

**Regras de Negócio**:
- Classificação de reportes (alerta→suspeito→confirmado)
- Tipos de usuário (normal vs admin)
