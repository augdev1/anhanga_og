...
from database import init_db, get_db, get_db_gen, UserORM, ReportORM, AuthTokenORM
...
@app.post("/auth/login")
def auth_login(payload: LoginPayload, db: Session = Depends(get_db_gen)):
    ...
    profile = {"username": username, "name": user.name or username}

    logger.info("AUTH_LOGIN username=%s", username)
    return {
        "status": "ok",
        "token": token,
        "token_type": "bearer",
...
def auth_register(payload: RegisterPayload, db: Session = Depends(get_db_gen)):
    ...
    logger.info("AUTH_REGISTER username=%s", username)
    return {
        "status": "ok",
        "token": token,
        "token_type": "bearer",
...
@app.get("/auth/me")
def auth_me(authorization: Optional[str] = Header(None), db: Session = Depends(get_db_gen)):
    token = _extract_bearer_token(authorization)
    if not token:
        raise HTTPException(status_code=401, detail="Token ausente")
    
    auth_token = db.query(AuthTokenORM).filter(AuthTokenORM.token == token).first()
    if not auth_token:
        raise HTTPException(status_code=401, detail="Token inválido")
    
    return {"status": "ok", "user": {"username": auth_token.username, "name": auth_token.name}}


@app.post("/auth/logout")
def auth_logout(authorization: Optional[str] = Header(None), db: Session = Depends(get_db_gen)):
    token = _extract_bearer_token(authorization)
    if token:
        db.query(AuthTokenORM).filter(AuthTokenORM.token == token).delete()
        db.commit()
    return {"status": "ok"}
...
def alerts_map(
    ...
    logger.info(
        ...
        days, confidence, start_date, end_date,
    )
...
def alerts_map_clusters(
    ...
    logger.info(
        ...
        days, confidence, start_date, end_date, eps_km, min_samples,
    )
...
def alerts_amazon_clustered(
    ...
    logger.info(
        ...
        days, confidence, eps_km, min_samples,
    )
...
@app.post("/reportar/queimada")
async def reportar_queimada(
    request: Request,
    lat: float = Query(..., description="Latitude"),
    lon: float = Query(..., description="Longitude"),
    authorization: Optional[str] = Header(None),
):
    """Recebe um reporte de queimada com foto e localização."""
    ...
    try:
        ...
        if token:
            ...
            if auth_token:
                ...
                name = auth_token.name or username
                # Try to get user_id
                user = db.query(UserORM).filter(UserORM.username == username).first()
                if user:
                    user_id = user.id
        ...
        # Count nearby reports (within ~0.01 degrees ≈ 1km)
        nearby_count = db.query(ReportORM).filter(
            (ReportORM.lat >= lat - 0.01) & (ReportORM.lat <= lat + 0.01) &
            (ReportORM.lon >= lon - 0.01) & (ReportORM.lon <= lon + 0.01)
        ).count()
        ...
        logger.info("REPORTAR lat=%s lon=%s user=%s nivel=%s count=%s", lat, lon, username, nivel, count)

        return {
            "status": "ok",
            "nivel": nivel,
            "usuarios": count,
            "message": f"Reporte registrado como {nivel}",
        }
    finally:
        db.close()


@app.get("/reportar/lista")
def reportar_lista(
    limit: int = Query(50, ge=1, le=200),
    db: Session = Depends(get_db_gen),
):
    """Lista reportes recentes."""
    recent = db.query(ReportORM).order_by(ReportORM.created_at.desc()).limit(limit).all()
    data = [
        {
            "id": r.id,
            "lat": r.lat,
